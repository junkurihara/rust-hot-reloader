...
use anyhow::anyhow;
use hot_reload::{RealtimeWatch, Reload, ReloaderService};
use std::fmt::Debug;
use std::sync::Arc;
use thiserror::Error;
...
impl Server {
  ...
  pub async fn entrypoint_with_realtime<T>(
    &self,
    context_reloader: ReloaderService<T, ServerConfig>,
  ) -> Result<(), ServerLibError>
  where
    T: RealtimeWatch<ServerConfig> + Clone + Send + Sync + Debug + 'static,
  {
    // Exit main thread if either of main service or reloader service exits.
    tokio::select! {
      // Main service
