...
impl<F> FileReloader<F> {
  ...
  async fn update_tracked_paths(&self, dependencies: Vec<PathBuf>) -> (Vec<PathBuf>, Vec<PathBuf>) {
  ...
  for<'a> <F as TryFrom<&'a PathBuf>>::Error: std::fmt::Display,
{
  ...
  async fn watch_realtime(&self) -> Result<RealtimeWatchHandle<F>, ReloaderError<F, String>> {
        ...
        handle.spawn(async move {
          ...
          let event = match res {
            Ok(event) => {
              ...
              let normalized_paths: Vec<PathBuf> = event.paths.iter().map(|p| normalize_event_path(p)).collect();
              let is_relevant = {
    ...
    Ok(RealtimeWatchHandle::with_cleanup(rx, Box::new(watcher_state)))
  }
}
