...
impl<V> RealtimeWatchHandle<V> {
    ...
    pub fn with_cleanup(
        ...
        cleanup: Box<dyn std::any::Any + Send>,
    ) -> Self {
        Self {
            rx,
    ...
    S: Into<std::borrow::Cow<'static, str>> + std::fmt::Display,
{
    ...
    async fn start_hybrid(&self) -> ReloadResult<(), V, S> {
        ...
        loop {
            ...
            if pending_handle.is_none() {
                ...
                if !self
                    .execute_reload_cycle("hybrid pre-realtime reload")
                    .await?
                {
                    return Ok(());
                }
                ...
                match self.reloader.watch_realtime().await {
                    ...
                    Err(e) => {
                        warn!(
                            "Failed to establish realtime watching: {}. Entering polling fallback.",
                            e
                        );
            ...
            if let Err(e) = self.run_realtime_loop(&mut handle).await {
                warn!(
                    "Realtime monitoring interrupted: {}. Switching to polling fallback.",
                    e
                );
    ...
    async fn run_realtime_loop(
        &self,
        handle: &mut RealtimeWatchHandle<V>,
    ) -> ReloadResult<(), V, S> {
        loop {
            match handle.rx.recv().await {
                ...
                None => {
                    ...
                    return Err(ReloaderError::Other(anyhow!(
                        "Realtime watch channel closed"
                    )));
                }
            }
        }
    ...
    async fn run_polling_fallback(&self) -> ReloadResult<Option<RealtimeWatchHandle<V>>, V, S> {
        ...
        loop {
            ...
            match self.reloader.watch_realtime().await {
                ...
                Err(e) => {
                    warn!(
                        "Realtime watching still unavailable: {}. Retrying in {:?}",
                        e, current_delay
                    );
                }
            }
